...
class Paginator{
	...
	// DECLARAMOS AQUÍ la propiedad que antes se creaba dinámicamente:
	public $default_ipp;
	...
	public function __construct(
		...
		$this->default_ipp = $this->ipp_array[0]; // ahora funciona sin generar dynamic property
		...
		if($this->num_pages > 10) {
			$this->return = ($this->current_page > 1 And $this->total_items >= 10) ? "<a class=\"paginate\" href=\"$servidor?page=".($this->current_page-1)."&ipp=$this->items_per_page$this->querystring\">«</a> ":"<span class=\"inactive\" href=\"#\">«</span> ";
			...
			for($i=1;$i<=$this->num_pages;$i++) {
				...
				if($i==1 Or $i==$this->num_pages Or in_array($i,$this->range)) $this->return .= ($i == $this->current_page And $this->items_per_page != "All") ? "<a title=\"Ir a la página $i of $this->num_pages\" class=\"current\" href=\"#\">$i</a> \n":"<a class=\"paginate\" title=\"Ir a la página $i of $this->num_pages\" href=\"$servidor?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a> \n";
				if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= " ... ";
			}
			$this->return .= (($this->current_page < $this->num_pages And $this->total_items >= 10) And ($this->items_per_page != "All") And $this->current_page > 0) ? "<a class=\"paginate\" href=\"$servidor?page=".($this->current_page+1)."&ipp=$this->items_per_page$this->querystring\">»</a>\n":"<span class=\"inactive\" href=\"#\">»</span>\n";
			
		} else	{
			for($i=1;$i<=$this->num_pages;$i++) {
				$this->return .= ($i == $this->current_page) ? "<a class=\"current\" href=\"#\">$i</a> ":"<a class=\"paginate\" href=\"$servidor?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a> ";
			}
	...
	public function display_items_per_page() {
		...
		return "<span class=\"paginate\">Items per page:</span><select class=\"paginate\" onchange=\"window.location='$servidor?page=1&amp;ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>\n";
	}
	...
	public function display_jump_menu() {
		...
		return "<span class=\"paginate\">Page:</span><select class=\"paginate\" onchange=\"window.location='$servidor?page='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
