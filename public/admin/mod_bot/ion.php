<?php
/*
Invoke-RestMethod -Method POST "http://localhost:84/bd_barsi/render/bar_cant_msg" `
...
Flight::route('POST /render/fich/listar', function () {
    ...
    // Prefijo de la mini según tu vari()
    $pic_fich = vari('FOTO_FICH_MAX');

    $rows = DB::query("
      SELECT
        f.fich_id,
        -- nueva columna: URL completa de la última foto mini
        COALESCE((
          SELECT CONCAT('$pic_fich', f.usu_id , '/' , fx.img)
          FROM fotoxfich AS fx
        ...
        u.provincia
      FROM fich AS f
      LEFT JOIN usu AS u ON u.usu_id = f.usu_id
    ");
    Flight::json($rows);
});

Flight::route('POST /ion/tk_ficha_fichera', function() {
    include DEFINITION;

    $req     = Flight::request()->data->getData();
    $fich_id = intval($req['fich_id'] ?? 0);
    $uid     = intval($req['usu_id'] ?? 0);
    $zz      = trim($req['zz'] ?? '');

    // ==========================================
    // BASE URL para fotos de la fichera
    // ==========================================
    ...
    $sql = "
      SELECT
        ...
        GROUP_CONCAT(
          DISTINCT CONCAT('{$url_img}/', ff.img)
        ...
        COALESCE(
          (SELECT CAST(fx.fichxubi_id AS CHAR)
             FROM fichxubi fx
             WHERE fx.fich_id = f.fich_id
        ...
        COALESCE(
          (SELECT fx3.map_lat
             ...
             ORDER BY fx3.fecha_creacion DESC
             LIMIT 1),
          ''
        ) AS map_lat,
        ...
        COALESCE(
          (SELECT fx6.ciudad
             ...
             ORDER BY fx6.fecha_creacion DESC
             LIMIT 1),
          ''
        ) AS ciudad
      ...
      WHERE f.fich_id = %i
      GROUP BY f.fich_id, u.usu_id, u.sobrenombre, u.descripcion
    ";

    $row = DB::queryFirstRow($sql, $fich_id);

    if (!$row) {
        Flight::halt(404, json_encode([
            'error' => "Ficha #{$fich_id} no encontrada"
        ]));
        return;
    }

    // ==========================================
    // 2) TIEMPO Y PRECIO DE SERVICIOS (servicios_det)
    // ==========================================
    $serv_det = DB::query("
        SELECT 
            s.serv_id,
            ...
            fs.precio
        FROM fichxserv fs
        INNER JOIN serv s ON s.serv_id = fs.serv_id
        WHERE fs.fich_id = %i
        ORDER BY s.descripcion
    ", $fich_id);

    $row['servicios_det'] = $serv_det;
    ...
    $comentarios = DB::query("
        SELECT
            ...
            fr.usu_id,
            u.sobrenombre AS usu_alias,
            u.celular AS usu_celular,
            fr.fecha_visita,
            fr.overall_score,
            fr.is_publicado,
            fr.comentario,
            ...
            fr.updated_at
        FROM fich_resena fr
        LEFT JOIN usu u ON u.usu_id = fr.usu_id
        LEFT JOIN neg n ON n.neg_id = fr.neg_id
        WHERE fr.fich_id = %i
        ORDER BY fr.fich_resena_id DESC
        LIMIT 20
    ", $fich_id);
