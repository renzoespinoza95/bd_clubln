...
Flight::route('GET /producto/listar', function () {
    $sql = "
      ...
      FROM product p
      LEFT JOIN inventario i ON i.product_id = p.product_id AND i.borrado_el IS NULL
...
Flight::route('POST /compra/agregar-items', function () {
    ...
    try {
        ...
        DB::query("
            UPDATE compra
            SET total_compra = (
              SELECT SUM(subtotal) FROM compra_detalle WHERE compra_id = %i AND borrado_el IS NULL
            )
            WHERE compra_id = %i
